...
export function formatOrder(order: OrderData, format: 'vk' | 'tg' = 'vk'): string {
    ...
    // === Цена ===
    const priceText = order.isRange && order.priceMin != null && order.priceMax != null && order.priceMin !== order.priceMax
        ? `от ${order.priceMin.toLocaleString('ru-RU')} до ${order.priceMax.toLocaleString('ru-RU')} ₽`
        : `${order.totalPrice.toLocaleString('ru-RU')} ₽`;
    parts.push(`💰 ${b('Итого:')} ${priceText}`);
